...
const MENU_ITEMS = [
  ...
  { 
    ...
    subItems: [
      { name: '입사 자원 할당', path: '/dashboard/hr-events/onboarding' },
      { name: '퇴사 자원 회수', path: '/dashboard/hr-events/offboarding' },
      { name: '부서 이동 자원 조정', path: '/dashboard/hr-events/transfer' },
    ]
  },
  { 
    name: 'IT 자산 관리', 
    ...
    subItems: [
      { name: '자산 재고 및 목록', path: '/dashboard/assets/inventory' },
  ...
  { 
    ...
    path: '/dashboard/saas',
    subItems: [
      { name: '라이선스 사용 현황', path: '/dashboard/saas/usage' },
            ...
            return (
              <div key={item.name} className="flex flex-col space-y-1 mb-1">
                <div 
                  ...
                  className={`flex items-center justify-between px-3 py-3 rounded-lg transition-colors cursor-pointer ${
                    ...
                    {item.subItems!.map((subItem) => {
                      ...
                      return (
                        <Link
                          key={subItem.name}
                          href={subItem.path}
                          className={`text-sm py-2 px-3 rounded-md transition-colors flex items-center before:content-[''] before:w-1 before:h-1 before:rounded-full before:mr-3 ${
                            isSubActive 
